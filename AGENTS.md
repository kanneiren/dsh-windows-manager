...
- WSL support targets WSL2 first. Never install or run Manager software inside WSL: the only WSL-side components are DSH and its generated Runtime Bridge `--patch`. Drive all management from the Windows Manager through `wsl.exe`.
- WSL support targets WSL2 first. Never assume a fixed distro such as Ubuntu: discover with `wsl.exe --list --quiet` plus `wsl.exe --list --verbose`, ignore Docker Desktop/Rancher/Podman helper distros, auto-select by configured/default/running/known-distribution order, and persist the selected `WslDistro`; require `--distro` only when general-purpose candidates remain ambiguous. Never guess Linux PIDs through WMI; use `wsl.exe` as the process handle and the Runtime Bridge as the authoritative PID/state source.
- WSL is disabled by default and detection is explicit (`wsl detect/enable/disable/status` or `configure`); never call `wsl.exe` on a background timer.
