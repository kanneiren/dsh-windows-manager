...
The event-driven refactor eliminated the steady fallback probes from the normal bridge-connected path, which removed the remaining periodic CPU wakeups: measured average CPU dropped from 0.103% to 0.000% on the one-core equivalent. The higher memory, handle, and thread counts are the measured cost of the persistent authenticated named-pipe connection, its reader task, and a woken CLR server GC. The values are stable over repeated samples (no growth trend, no leak); the managed heap stays near 7 MB. Per the project goals, memory is not the primary optimization target and is not being traded for a lower CPU number.
